...
// verifica se um dos filhos é menor, se sim troca 
...
void corrigeD(int a[], int tam, int i){
    ...
    while (2*j<=tam){ // enquanto o 1st filho estiver dentro do limite
        f=2*j;
        // se 2nd filho estiver dentro do limite, verifica qual dos filhos é menor
        if (f+1<=tam && a[f] > a[f+1]){ 
            f=f+1;
        }
        // se o pai for menor que o maior filho, termina
        if (a[j] <= a[f]){
            j = tam;
        }
